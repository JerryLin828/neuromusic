"""
Full evaluation pipeline using PromptGeneratorV2 (continuous_v2 backend).

Identical to run_full_eval.py except:
  - Uses PromptGeneratorV2 from src.bridge.prompt_generator_v2
  - Writes results to eval_dir_v2 (default: evaluation/full_eval_continuous_v2_100)
    so it never overwrites the v1 continuous results.

Usage (on GPU machine):
    python -m evaluation.run_full_eval_v2
    python -m evaluation.run_full_eval_v2 --config configs/default.yaml
"""
...
def run_full_evaluation_v2(cfg: dict):
    paths = cfg.get("paths", {})
    eval_cfg = cfg.get("evaluation", {})
    musicgen_cfg = cfg.get("musicgen", {})

    data_dir = Path(paths.get("data_dir", "data/raw/dreamer"))
    # Write to a separate folder so v1 results are never overwritten
    output_dir = Path(
        paths.get("eval_dir_v2", "evaluation/full_eval_continuous_v2_100")
    )
    ...
    alpha = bridge_cfg.get("continuous", {}).get("alpha", 1.2)
    prompt_gen = PromptGeneratorV2(alpha=alpha)
    ...
    for cond_name in ["therapeutic", "random", "fixed_calm", "non_inverted"]:
        audio_paths = [output_dir / r["audio_files"][cond_name] for r in results]
    ...
    report = {
        "prompt_generator": "PromptGeneratorV2",
        "alpha": alpha,
        "n_samples": n_total,
    ...
    print(f"\n{'='*60}")
    print("EVALUATION SUMMARY  (PromptGeneratorV2)")
    print(f"{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(description="Full pipeline evaluation (v2 prompt generator)")
